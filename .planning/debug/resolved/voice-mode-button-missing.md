...
verification: `docker compose run --rm frontend pnpm build` passes cleanly (tsc -b + vite build, ✓ built in 4.60s, zero errors). Fix committed in 2634f17. Human confirmed TTS per-message play button works and approved this fix. Voice mode button now shows whenever TTS can play, regardless of STT availability.
