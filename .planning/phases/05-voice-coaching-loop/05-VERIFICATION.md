...
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `api/app/coaching/tts.py`                                | TTSService with speak(), lazy ElevenLabs import, sentence-boundary truncation              | ✓ VERIFIED | 54 lines; `TTSService(api_key)` constructor (no voice_id); `speak(text, voice_id, locale)` — voice_id is per-call parameter; raises `TTSUnavailableError` if voice_id is falsy  |
| `api/tests/test_tts.py`                                  | Unit tests for 200/503 behavior, text truncation, auth guard, loader                       | ✓ VERIFIED | 230 lines; 21 tests covering truncation (4), TTSService (5 incl. no-voice-id guard), get_voice_id loader (8), and endpoint integration (4); 21/21 pass                          |
...
Voice IDs are **no longer configured via `ELEVENLABS_VOICE_ID`**. They are resolved at request time from `api/app/personas/config.json` via `app.personas.loader.get_voice_id(persona_id, locale, gender)`. The lookup order is: named persona → locale bucket → `"any"` bucket → `defaultPersonaSettings`. No hardcoded voice IDs exist in `config.py`, `.env`, or `TTSService`.
