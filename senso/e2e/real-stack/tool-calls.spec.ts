...
test.describe("Coach tool calls (real stack)", () => {
    test("get_user_profile tool call: returns non-empty response", async ({
        page,
        account,
    }) => {
        ...
        await page.getByPlaceholder(/Chiedi al coach/i).fill(
            "get_user_profile - mostrami il mio profilo finanziario"
        )
    ...
    test("search_italy_rules tool call: response references IRPEF", async ({
        page,
        account,
    }) => {
        await loginAs(page, account)
        ...
        await page.getByPlaceholder(/Chiedi al coach/i).fill(
            "search_italy_rules - spiegami le aliquote IRPEF 2025"
        )
        await page.getByRole("button", { name: "Invia" }).click()
    ...
    test("resource_card URLs are from real catalog (not LLM-invented)", async ({
        page,
        account,
    }) => {
        await loginAs(page, account)
        ...
        await page.getByPlaceholder(/Chiedi al coach/i).fill(
            "Posso comprare un iPhone da 1200 euro?"
        )
        await page.getByRole("button", { name: "Invia" }).click()
