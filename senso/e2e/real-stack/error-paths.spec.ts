...
test.describe("Error paths (real stack)", () => {
    test("expired token: corrupted token redirects to login", async ({
        page,
        account,
    }) => {
    ...
    test("bad file upload: frontend shows error for bad file type", async ({
        page,
        account,
    }) => {
        await loginAs(page, account)
    ...
    test("LLM timeout: shows error message, no infinite spinner", async ({
        page,
        account,
    }) => {
        await loginAs(page, account)
        ...
        const spinnerLocator = page.locator(
            '[aria-live="polite"] .animate-bounce'
        )
