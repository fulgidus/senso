...
test.describe("Chat - swipe-up scroll regression @mobile", () => {
    ...
    test("swipe-down from top-30% zone triggers pull-to-refresh on profile @mobile", async ({ page }) => {
        await setupAuthenticatedSession(page)
        mockNotifications(page)
        mockProfile(page)
    ...
    test("swipe-down from mid-screen (>30%) does NOT trigger pull-to-refresh @mobile", async ({ page }) => {
        await setupAuthenticatedSession(page)
        mockNotifications(page)
        mockProfile(page)
        await page.goto("/profile")
