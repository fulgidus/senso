...
async function goToAdminContent(page: Parameters<typeof setupAdminSession>[0]) {
    ...
    await expect(
        page.getByRole("heading", { name: /content management/i })
...
test.describe("Smoke - Admin content page", () => {
    ...
    test("admin content page loads with item table", async ({ page }) => {
        ...
        await expect(
            page.getByText(/30 items/i).or(page.getByText(/items in catalog/i))
        ...
        page.route(/\/admin\/content\/items(?:\?|$)/, (route: Route) => {
            ...
            if (localeParam === "en") {
                route.fulfill({
                    ...
                    body: JSON.stringify({ items: enItems, total: 15, page: 1, page_size: 100, total_pages: 1 }),
                })
            } else {
                // Default: all 30
    ...
    test("New content button shows and hides the create form", async ({ page }) => {
        ...
        await expect(
            page.getByRole("heading", { name: /create new content/i })
        ).not.toBeVisible()
