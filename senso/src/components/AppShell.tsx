...
function NavItemLink({ item, onClick }: { item: NavItem; onClick?: () => void }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        [
          "ripple-target flex items-center gap-3 rounded-xl px-3 py-2.5 min-h-[48px] text-sm font-semibold transition-all",
          isActive
...
function TopBarNavLink({ item }: { item: NavItem }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        [
          "ripple-target flex items-center gap-1.5 rounded-full px-3 py-2 text-sm font-semibold whitespace-nowrap transition-all",
          isActive
...
export function AppShell({ children }: AppShellProps) {
  ...
  if (user && user.isAdmin && !user.adminHandle) {
    return <AdminHandleGateModal onSaved={(handle) => updateUser({ adminHandle: handle })} />;
  }
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-2 border-b border-border bg-background px-4">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-3">
          ...
          <Link
            to="/"
            className="rounded-full bg-gradient-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/30 transition-all hover:shadow-xl hover:shadow-primary/40 hover:brightness-110 active:scale-[0.98]"
          >
            {t("nav.loginCta")}
          </Link>
        </div>
