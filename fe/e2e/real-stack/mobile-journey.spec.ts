...
test.describe("Mobile journey - iPhone 14 (real stack)", () => {
  ...
  test("mobile: coach response does not overflow 390px", async ({ page, account }) => {
    await loginAs(page, account);
    await page.goto("/chat");
    ...
    // Send a message
    await page.getByPlaceholder(/Chiedi al coach/i).fill("Ho abbastanza per le vacanze?");
  ...
  test("mobile: voice button has ≥44px tap target (if present)", async ({ page, account }) => {
    await loginAs(page, account);
    await page.goto("/chat");
  ...
  test("mobile: profile tab grid visible at 390px", async ({ page, account }) => {
    await loginAs(page, account);
    await page.goto("/profile");

    await expect(page.getByRole("heading", { name: /profilo/i })).toBeVisible({ timeout: 10_000 });
  ...
  test("mobile: upload button accessible at 390px", async ({ page, account }) => {
    await loginAs(page, account);
