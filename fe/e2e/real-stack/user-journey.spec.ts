...
const FINECO_XLSX = path.resolve(__dirname, "../../../api/tests/fixtures/fineco_sample.xlsx");

test.describe("Full user journey (real stack)", () => {
  test("auth: login via UI with real API credentials", async ({ page, account }) => {
  ...
  test("upload: Fineco XLSX ingested successfully", async ({ page, account }) => {
    ...
    await expect(page.getByText(/fineco_sample|In attesa|In elaborazione|Completato/i)).toBeVisible(
      { timeout: 15_000 },
    );
  });

  test("profile: shows data after document upload", async ({ page, account }) => {
    await loginAs(page, account);
  ...
  test("coach: receives real LLM stub response to a question", async ({ page, account }) => {
    await loginAs(page, account);
