...
import { setupAuthenticatedSession, mockProfile, mockAllCoaching } from "./support/api-mocks";
...
test.describe("Smoke - Profile page sections & hash nav", () => {
  ...
  test("profile page loads and shows financial data sections", async ({ page }) => {
    ...
    await expect(page.locator("#income").or(page.getByText(/monthly income/i))).toBeVisible({
      timeout: 8_000,
    });
    ...
    // ── Insights section ──────────────────────────────────────────────────────
    await expect(page.locator("#insights").or(page.getByText(/what your data says/i))).toBeVisible({
      timeout: 8_000,
    });
  });
  ...
  test("profile stale-data warning is absent when data is fresh", async ({ page }) => {
    ...
    await expect(page.getByText(/newdocuments uploaded/i)).not.toBeVisible();
  });
