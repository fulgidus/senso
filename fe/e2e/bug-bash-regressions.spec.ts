...
test.describe("Questionnaire quick mode hidden", () => {
  test("onboarding choice screen does NOT show quick questionnaire option", async ({ page }) => {
    ...
    await expect(
      page
        .getByText(/in-depth/i)
        .or(page.getByText(/approfondito/i).or(page.getByText(/thorough/i))),
...
test.describe("Questionnaire step order", () => {
  test("currency step appears before income step", async ({ page }) => {
    ...
    await expect(page.getByText(/employment/i).or(page.getByText(/occupazione/i))).toBeVisible({
      timeout: 8_000,
    });
    // Click "Employed" or first option
    const employedBtn = page
      .locator("button")
      .filter({ hasText: /employed|dipendente/i })
      .first();
    ...
    await expect(
      page.locator("select option[value='EUR']").or(page.getByText(/currency|valuta/i)),
    ...
    await expect(
      page
        .getByText(/income source|fonte.*reddito/i)
        .or(page.getByText(/add.*source|aggiungi.*fonte/i)),
    ).toBeVisible({ timeout: 5_000 });
  });
});
...
test.describe("Toast positioning", () => {
  test("LocationToast renders at the top, is non-interactive, and auto-dismisses", async ({
    page,
  }) => {
    await setupAuthenticatedSession(page);
    mockAllCoaching(page);
...
test.describe("Deep about page", () => {
  test("/about/deep loads without authentication", async ({ page }) => {
    ...
    await expect(page.getByText(/technical detail|dettagli tecnici/i)).toBeVisible({
      timeout: 8_000,
    });

    // Architecture section visible
    await expect(page.getByText(/architecture|architettura/i).first()).toBeVisible();

    // Privacy section visible
    await expect(page.getByText(/privacy.*security|privacy.*sicurezza/i).first()).toBeVisible();
