      ...
      <div className="snap-start shrink-0 w-56 border border-primary/30 rounded-lg overflow-hidden bg-primary/5">
        ...
        <div className="px-3 py-2">
          ...
          <p className="text-xs text-muted-foreground line-clamp-2 mt-0.5">
            {card.plain_explanation ?? card.summary}
          </p>
          {card.example && (
            <p className="text-xs text-muted-foreground italic mt-1 line-clamp-2">{card.example}</p>
          )}
        </div>
      </div>
    );
...
function DebugPanel({ debug }: { debug: DebugPayload }) {
  ...
  return (
    <div className="mt-3 space-y-1 border-t-2 border-yellow-400 pt-2">
          ...
          <>
            <div>
              ...
              <span className="font-semibold">
                {trace.provider_used}/{trace.model_used}
              </span>
            ...
            {trace.tokens.total != null && (
              <div>
                Tokens: <span className="font-semibold">{trace.tokens.total}</span> (prompt:{" "}
                {trace.tokens.prompt ?? "?"} · completion: {trace.tokens.completion ?? "?"})
              </div>
            )}
            ...
            {trace.provider_errors.length > 0 && (
              <div className="text-red-600">Errors: {trace.provider_errors.join(" | ")}</div>
            )}
          </>
        )}
      </div>
      ...
      />
      {trace && <DebugSection label="Call Trace" content={JSON.stringify(trace, null, 2)} />}
    </div>
  );
}
  ...
  return (
    <div className="relative inline-flex items-center gap-1">
      <Button
        ...
        aria-label={
          isGenerating
            ? t("coaching.ttsGenerating")
            : isPlaying
              ? t("coaching.ttsPlaying")
              : t("coaching.ttsPlay")
        }
        title={
          showFallbackBadge
            ? t("coaching.ttsFallbackHint")
            : isGenerating
              ? t("coaching.ttsGeneratingShort")
              : isPlaying
                ? t("coaching.ttsPlayingShort")
      ...
      >
        ...
        {resp && (
          <div className="animate-in fade-in duration-200 slide-in-from-bottom-1 motion-reduce:animate-none">
            ...
            {/* 4. Goal progress */}
            {resp.goal_progress && <GoalProgressBar progress={resp.goal_progress} />}
...
export function ChatScreen({
  ...
  const [personaDefaultGender, setPersonaDefaultGender] = useState<
    ...
    try {
      ...
      if (err instanceof CoachingApiError) {
        ...
        setErrorWithAutoDismiss({
          code: "network_error",
          message: getErrorMessage("network_error", t),
        });
      }
      return [];
    }
    ...
    void (async () => {
      ...
      try {
        ...
        const active =
          ...
          personas.find((p) => p.id === "mentore-saggio") ??
          personas[0];
        ...
        /* non-fatal - use DEFAULT_TTS_CONFIG */
      }
    ...
    async (text?: string) => {
      ...
      try {
        ...
        try {
          ...
          const fallback = await coachingApi.sendMessage(
            trimmed,
            locale,
            activePersonaId,
            sessionId,
            ...
            void generateConversationName(trimmed).then(async (name) => {
              ...
              if (name && name.trim()) {
                try {
                  ...
                  setSessions((prev) => prev.map((s) => (s.id === newSessionId ? updated : s)));
  ...
  const handleRetry = useCallback(
    (failedMessageIndex: number) => {
      ...
      setMessages((prev) =>
        prev.map((msg, i) => (i === failedMessageIndex ? { ...msg, status: undefined } : msg)),
      );
      ...
      void (async () => {
        try {
          ...
          try {
            finalResponse = await sendMessageStream(
              failedMsg.content,
              locale,
              activePersonaId,
              sessionId,
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="bg-muted rounded-2xl rounded-tl-sm px-4 py-3 text-sm text-muted-foreground">
                <span className="animate-pulse">
                  {t(`coaching.thinking.${effectiveGender}`, { name: personaName })}
                </span>
              </div>
            </div>
          )}
      ...
      {error && (
        <div className="px-4 py-2 bg-destructive/10 border-t border-destructive/20 shrink-0">
          ...
          {error.code === "profile_required" && (
            <button onClick={onNavigateBack} className="text-xs text-destructive underline mt-1">
      ...
      >
        ...
        {isVoiceMode ? (
          <VoiceModeBar
            ...
            sttError={
              sttError && sttErrorCode && !STT_HARD_ERROR_CODES.has(sttErrorCode) ? sttError : null
            }
        ...
        <p className="text-xs text-muted-foreground mt-1 text-center">{t("coaching.disclaimer")}</p>
      </div>
