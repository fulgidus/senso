    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground">
          {t("questionnaire.expenseRangeToggle")}
        </span>
      </div>
    </div>
  );
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[13, 14].map((m) => (
                ...
                >
                  {m}
                  {t("questionnaire.incomeExtraMonthsLabel")}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-lg border border-border bg-secondary/30 p-3 space-y-2">
              ...
              <div>
                <label className="block text-xs text-muted-foreground mb-1">
                  ...
                  <span className="text-amber-500">
                    {t("questionnaire.incomeMealVoucherDaysHint")}
                  </span>
                </label>
                <input
                  type="number"
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() =>
                  handleVoucherChange("mealVoucherElectronic", !source.mealVoucherElectronic)
                }
            ...
            {(source.mealVoucherFaceValue ?? 0) > 0 &&
              (source.mealVoucherEstimatedDaysMonth ?? 0) > 0 &&
              net && (
                <p className="text-xs text-muted-foreground">
                  ...
                  <span className="ml-1 text-amber-500">
                    {t("questionnaire.incomeMealVoucherVariable")}
                  </span>
                </p>
              )}
          </div>
            ...
            <div className="rounded-lg bg-primary/5 border border-primary/20 p-3 space-y-1">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-foreground">
                  {currencySymbol}
                  {fmt.number(net.netMonthly, { maximumFractionDigits: 0 })}
                </span>
              </div>
              {net.mealVoucherMonthlyEstimate > 0 && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-foreground">
                    +{currencySymbol}
                    {fmt.number(net.mealVoucherMonthlyEstimate, { maximumFractionDigits: 0 })}
                  </span>
                </div>
              )}
              <div className="flex justify-between text-xs text-muted-foreground border-t border-border pt-1 mt-1">
                ...
                <span>
                  -{currencySymbol}
                  {fmt.number(net.inpsAnnual, { maximumFractionDigits: 0 })}
                  {t("profile.perYear")}
                </span>
              </div>
              <div className="flex justify-between text-xs text-muted-foreground">
                ...
                <span>
                  -{currencySymbol}
                  {fmt.number(net.irpefNet, { maximumFractionDigits: 0 })}
                  {t("profile.perYear")}
                </span>
              </div>
              <div className="flex justify-between text-xs text-muted-foreground">
                ...
                <span>
                  -{currencySymbol}
                  {fmt.number(net.addizionali, { maximumFractionDigits: 0 })}
                  {t("profile.perYear")}
                </span>
              </div>
            </div>
            ...
            {source.isFixed ? (
              ...
              <>
                ...
                <DualSlider
                  ...
                  onChange={([lo, hi]) =>
                    onChange({ ...source, valueMin: lo, valueMax: Math.max(lo, hi) })
                  }
                />
              </>
            )}
...
export function QuestionnaireScreen({ user: _user, token, mode, onComplete, onBack }: Props) {
  ...
  const addIncomeSource = () => {
    setAnswer("incomeSources", [...incomeSources, makeIncomeSource({ currency, label: "" })]);
  };
      ...
      case 2:
        return (
          <div>
            ...
            <p className="mb-4 text-sm text-muted-foreground">{t("questionnaire.step1Subtitle")}</p>
      ...
      case 3: {
        ...
        return (
          <div>
            ...
            <p className="mb-4 text-sm text-muted-foreground">{t("questionnaire.step3Subtitle")}</p>
            <div className="space-y-5">
              {EXPENSE_CATEGORIES.map((cat) => (
                <ExpenseCategoryRow
                  ...
                  valueRange={
                    catRanges[cat.key] ?? { min: cats[cat.key] ?? 0, max: cats[cat.key] ?? 0 }
                  }
