...
function TraceBadge({ status }: { status: string }) {
  ...
  const cls = classMap[status] ?? "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400";
  return (
...
export function AdminInspectorDrawer({ uploadId, token, onClose }: Props) {
  ...
  const [extracted, setExtracted] = useState<ExtractedDocumentDetail | null | undefined>(undefined);
  ...
  useEffect(() => {
    setLoading(true);
    void Promise.all([
      filesApi.getExtracted(token, uploadId),
      filesApi.getTrace(token, uploadId),
    ]).then(([ext, tr]) => {
      setExtracted(ext);
      setTrace(tr);
      setLoading(false);
      ...
      <div className="mx-auto w-full max-w-4xl space-y-4 px-6 py-6">
        {loading ? (
          ...
          <>
            ...
            <Section title={t("inspector.sectionUpload")} copyData={{ uploadId }}>
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <dd className="text-foreground">
                        {fmt.date(extracted.extracted_at, {
                          dateStyle: "medium",
                          timeStyle: "short",
                        })}
                      </dd>
            ...
            <Section title={t("inspector.sectionExtracted")} copyData={extracted ?? undefined}>
              {extracted === null ? (
                ...
                <dl className="grid grid-cols-1 gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
                  ...
                  <div>
                    <dt className="text-xs text-muted-foreground">{t("inspector.confidence")}</dt>
                    <dd className="text-foreground">{(extracted.confidence * 100).toFixed(1)}%</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-muted-foreground">extracted_at</dt>
                    <dd className="text-foreground">
            ...
            <Section title={t("inspector.sectionPayload")} copyData={extracted?.payload_json}>
              {extracted ? (
