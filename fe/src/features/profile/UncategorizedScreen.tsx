...
import { createProfileApi, type UncategorizedTransactionDTO } from "@/lib/profile-api";
...
function looksLikePrivateIndividual(name: string): boolean {
  ...
  const businessIndicators =
    /\b(srl|spa|sas|snc|banca|bank|poste|amazon|google|apple|netflix|spotify|telecom|vodafone|tim|enel|eni|fineco|unicredit|intesa|sanpaolo|bnl|bnp|paypal|stripe|klarna)\b/i;
...
function DirectionIcon({ type }: { type: ActorGroup["dominantType"] }) {
  if (type === "income") return <ArrowDownLeft className="h-4 w-4 text-green-500 shrink-0" />;
  if (type === "expense") return <ArrowUpRight className="h-4 w-4 text-red-400 shrink-0" />;
...
export function UncategorizedScreen() {
  ...
  const handleBulkCategorize = useCallback(
    async (description: string, category: string) => {
      ...
      try {
        const result = await profileApi.bulkUpdateCategoryByDescription(
          token,
          description,
          category,
        );
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-6 py-6">
      ...
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-foreground">{t("uncategorized.pageTitle")}</h2>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {group.counterpart_name && group.counterpart_name !== group.description && (
