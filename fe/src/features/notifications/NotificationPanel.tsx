...
import { AlertTriangle, Clock, ShieldOff, CheckCircle, RotateCcw, Bell } from "lucide-react";
import {
  getNotifications,
    ...
    async (notif: NotificationDTO) => {
      if (!notif.is_read) {
        try {
          ...
          setData((prev) => {
            if (!prev) return prev;
            const items = prev.items.map((n) => (n.id === notif.id ? { ...n, is_read: true } : n));
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border sticky top-0 bg-background z-10">
        <h3 className="text-sm font-semibold text-foreground">{t("notifications.panelTitle")}</h3>
        {unreadCount > 0 && (
          <button
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-muted-foreground line-clamp-2 mt-0.5">{notif.body}</p>
