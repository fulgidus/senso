...
export function DebugScreen() {
  ...
  const callDebug = async (action: string, method: "POST" | "DELETE", path: string) => {
    setLoading((prev) => ({ ...prev, [action]: true }));
    try {
      const data = await apiRequest<Record<string, unknown>>(getBackendBaseUrl(), path, {
        method,
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-6 py-10 space-y-8">
      ...
      <section className="rounded-2xl border border-border bg-card p-6 space-y-3">
        <h2 className="text-base font-semibold text-foreground">{t("debug.restartTitle")}</h2>
        ...
        {results["restart"] && (
          <p className="text-xs font-mono text-muted-foreground break-all">{results["restart"]}</p>
        )}
      </section>

      {/* Section 2 - Purge coaching history */}
      <section className="rounded-2xl border border-border bg-card p-6 space-y-3">
        <h2 className="text-base font-semibold text-foreground">{t("debug.purgeTitle")}</h2>
        ...
        {results["purge"] && (
          <p className="text-xs font-mono text-muted-foreground break-all">{results["purge"]}</p>
        )}
      </section>

      {/* Section 3 - Nuclear option */}
      <section className="rounded-2xl border border-destructive/30 bg-card p-6 space-y-3">
        <h2 className="text-base font-semibold text-foreground">{t("debug.nukeTitle")}</h2>
        ...
        {results["nuke"] && (
          <p className="text-xs font-mono text-muted-foreground break-all">{results["nuke"]}</p>
        )}
      </section>
      ...
      {isAdmin && (
        <section className="rounded-2xl border-2 border-destructive bg-card p-6 space-y-3">
          <h2 className="text-base font-semibold text-destructive">{t("debug.nukeAllTitle")}</h2>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          void callDebug("nukeAll", "DELETE", "/debug/nuke-all");
        }}
