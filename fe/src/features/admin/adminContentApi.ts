...
export async function deleteContentItem(id: string): Promise<{ deleted: boolean }> {
...
export function createAdminContentApi(onUnauthorized?: () => Promise<string | null>) {
  ...
  return {
    ...
    getItemSiblings: (itemId: string) =>
      req<AdminContentItemDTO[]>(`/admin/content/items/${encodeURIComponent(itemId)}/siblings`),
    ...
    deleteContentItem: (id: string) =>
      req<{ deleted: boolean }>(`/admin/content/items/${encodeURIComponent(id)}`, {
        method: "DELETE",
      }),

    bulkPublish: (params: { itemIds: string[]; isPublished: boolean; applyToGroup?: boolean }) =>
      req<{ updated: number; groups_affected: number }>("/admin/content/bulk-publish", {
        method: "POST",
        body: {
          item_ids: params.itemIds,
          is_published: params.isPublished,
    ...
    bulkDelete: (params: { itemIds: string[]; applyToGroup?: boolean }) =>
      req<{ deleted: boolean }>("/admin/content/bulk-delete", {
        method: "POST",
        body: {
          item_ids: params.itemIds,
          apply_to_group: params.applyToGroup ?? true,
