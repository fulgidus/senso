...
export async function getModerationQueue(statusFilter?: string): Promise<ModerationLogAdminDTO[]> {
  ...
  return apiRequest<ModerationLogAdminDTO[]>(API_BASE, `/admin/moderation${qs}`, {
    token: requireToken(),
  });
}

export async function confirmModerationAction(logId: string): Promise<void> {
  await apiRequest<void>(API_BASE, `/admin/moderation/${encodeURIComponent(logId)}/confirm`, {
    method: "POST",
    token: requireToken(),
  });
}
...
export async function revertModerationAction(logId: string): Promise<void> {
  await apiRequest<void>(API_BASE, `/admin/moderation/${encodeURIComponent(logId)}/revert`, {
    method: "POST",
    token: requireToken(),
  });
}
...
export function createAdminMerchantApi(onUnauthorized?: () => Promise<string | null>) {
  ...
  return {
    ...
    blacklistMerchant: (merchantId: string, reason: string) =>
      req<void>(`/admin/learned-merchants/${encodeURIComponent(merchantId)}/blacklist`, {
        method: "POST",
        body: { reason },
      }),

    unblacklistMerchant: (merchantId: string) =>
      req<void>(`/admin/learned-merchants/${encodeURIComponent(merchantId)}/unblacklist`, {
        method: "POST",
      }),
    ...
    confirmModerationAction: (logId: string) =>
      req<void>(`/admin/moderation/${encodeURIComponent(logId)}/confirm`, { method: "POST" }),

    revertModerationAction: (logId: string) =>
      req<void>(`/admin/moderation/${encodeURIComponent(logId)}/revert`, { method: "POST" }),
