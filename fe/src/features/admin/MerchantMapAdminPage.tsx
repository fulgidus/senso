...
import { createAdminMerchantApi, type MerchantMapAdminDTO } from "./adminMerchantApi";
...
function MethodBadge({ method }: { method: string }) {
  const cls =
    method === "manual" ? "bg-primary/10 text-primary" : "bg-secondary text-secondary-foreground";
  return (
    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${cls}`}>
      {method}
    </span>
...
export function MerchantMapAdminPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">{t("admin.merchantMap.title")}</h1>
      </div>
