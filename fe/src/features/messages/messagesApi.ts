...
export function createMessagesApi(onUnauthorized?: () => Promise<string | null>) {
  ...
  return {
    ...
    getAttachmentDownloadUrl: async (attachmentId: string): Promise<{ presignedUrl: string }> => {
      const result = await req<{ presigned_url: string }>(`/attachments/${attachmentId}/download`);
      return { presignedUrl: result.presigned_url };
    },
