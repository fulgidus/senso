...
style: |
  ...
  section.lt td { color: #112D4E; border-color: #DBE2EF; }
...
> Imparano facendo scelte (e spesso sbagliando).
> Noi interveniamo esattamente lì.
...
- **7 Tool LLM** - profilo, transazioni, conoscienza regionale, memoria, timeline, preferenze, contenuti
- **Structured Output** - `response_format` `JSON Schema`, no `prose`
- **Ragionamento Trasparente** - ogni risposta mostra come si è formata
- **Voice In / Voice Out** - ElevenLabs TTS/STT/STS con voci impostabili, fallback automatico
- **Safety stratificata** - input guard (LLM + RegEx) → persona boundaries (LLM) → output guard (LLM + RegEx)
- **Classificazione Merchant** - regole → LLM small → LLM large + mappa crowdsourced
- **Knowledge Base Regionale** - Per ora `it` (INPS, 730, TFR, etc...), ma scalabile a qualsiasi paese o lingua
...
| :---: | -------------------- | -------------------- | ----------------------------------------------------- |
|   🦉   | **Mentore Saggio**   | Calmo, diretto       | *"Con 300€ di margine, le rate saranno un problema."* |
...
> \* feature futura: diversi framework di risparmio/budgeting/investimento consigliati a seconda della persona (50-30-20, 15-65-20, 80-20, Zero-based, etc...)
...
| Layer         | Tecnologie                                                                              |
| ------------- | --------------------------------------------------------------------------------------- |
| **Frontend**  | **TypeScript**: React 19 + Vite · Tailwind 4 + shadcn/ui · **PWA standalone**           |
| **Backend**   | **Python**: FastAPI + Pydantic · SQLAlchemy ORM · PostgreSQL                            |
| **AI**        | Provider agnostic (io uso Open Router) · BM25 retrieval · Jinja2 prompt templates       |
| **Voice**     | ElevenLabs TTS · Web Speech API STT + Browser TTS (optional fallback)                   |
| **Sicurezza** | PII decoupling · AES-GCM encryption at rest · PBKDF2 · NaCl E2E messaging · ZDR headers |
| **Ingestion** | OCR 3-tier (Tesseract → LLM text → LLM vision) · 7 moduli parser                        |
| **Infra**     | Docker Compose → MinIO S3 · PostgreSQL · Nginx · Let's Encrypt TLS                      |
...
- Connettori bancari diretti (plaid.com, etc...)
- *Fraud & Scam Gym*
